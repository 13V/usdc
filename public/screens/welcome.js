...
(function () {
    ...
    return '' +
      '<div style="position:relative; width:200px; height:200px; display:flex; align-items:center; justify-content:center; margin-bottom:6px;">' +
        ...
        '<div style="position:relative; display:flex; align-items:center; justify-content:center;">' +
          ...
          '<div style="position:absolute; right:24px; bottom:28px; width:40px; height:40px; border-radius:50%; background:linear-gradient(150deg,#5ff0d4,#2bccae); border:3px solid var(--border-ink); display:flex; align-items:center; justify-content:center; box-shadow:3px 3px 0 rgba(var(--shadow-rgb),0.9); animation:wrPop .5s ease-out both; animation-delay:.35s;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--border-ink)" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></div>' +
        '</div>' +
      '</div>';
  }
  ...
  function paint(view, user) {
    ...
    view.innerHTML = '' +
      '<div class="vfill" style="position:relative; display:flex; flex-direction:column; background:var(--paper); overflow:hidden;">' +
        ...
        '<div style="position:relative; z-index:6; padding:8px 16px 2px; flex:none;">' +
          '<div id="wClose" role="button" aria-label="close" tabindex="0" style="width:36px; height:36px; border-radius:50%; background:rgba(var(--ink-rgb),0.05); border:1px solid rgba(var(--ink-rgb),0.08); display:flex; align-items:center; justify-content:center; cursor:pointer;"><svg aria-hidden="true" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="rgba(var(--ink-rgb),0.6)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg></div>' +
        '</div>' +
        ...
        '<div style="position:relative; z-index:2; flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:0 26px 8px; text-align:center;">' +
          ...
          '<h1 style="font-family:\'Clash Display\',\'General Sans\',sans-serif; font-weight:600; font-size:46px; letter-spacing:-1.4px; margin:8px 0 0; color:var(--ink);">' + headline + '</h1>' +
          '<p style="font-family:\'General Sans\',sans-serif; font-weight:400; font-size:14.5px; line-height:1.5; color:rgba(var(--ink-rgb),0.55); max-width:300px; margin:12px 0 0; text-wrap:pretty;">' + sub + '</p>' +

          // wallet identity card
          '<div style="width:100%; max-width:320px; margin-top:26px;">' +
            '<div style="font-family:\'Space Mono\',monospace; font-size:10px; letter-spacing:1.5px; color:rgba(var(--ink-rgb),0.42); text-align:left; margin:0 4px 9px;">YOUR ACCOUNT</div>' +
            '<div style="display:flex; align-items:center; gap:13px; background:var(--card); border:2px solid var(--border-ink); border-radius:20px; box-shadow:3px 4px 0 rgba(var(--shadow-rgb),0.85), 0 14px 32px rgba(var(--shadow-rgb),0.10); padding:14px 15px;">' +
              '<div style="width:46px; height:46px; border-radius:15px; background:' + color + '; display:flex; align-items:center; justify-content:center; font-size:24px; flex:none; box-shadow:3px 3px 0 rgba(var(--shadow-rgb),0.9);">' + app.face(emoji) + '</div>' +
              '<div style="flex:1; text-align:left; min-width:0;">' +
                '<div style="font-family:\'Space Mono\',monospace; font-size:9px; letter-spacing:1px; color:rgba(var(--ink-rgb),0.4);">' + idLabel + '</div>' +
                '<div style="font-family:\'Space Mono\',monospace; font-weight:700; font-size:17px; letter-spacing:.5px; color:var(--ink); margin-top:3px;">' + app.esc(trunc(wallet)) + '</div>' +
              '</div>' +
              '<div id="wCopy" role="button" aria-label="copy address" tabindex="0" title="copy address" style="width:38px; height:38px; border-radius:11px; background:rgba(var(--ink-rgb),0.05); border:1px solid rgba(var(--ink-rgb),0.08); display:flex; align-items:center; justify-content:center; cursor:pointer; flex:none;"><svg aria-hidden="true" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(var(--ink-rgb),0.55)" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="11" height="11" rx="2.5"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg></div>' +
            '</div>' +
            '<div style="display:flex; align-items:center; justify-content:center; gap:7px; margin-top:12px;">' +
              ...
              '<span style="font-family:\'Space Mono\',monospace; font-size:10.5px; letter-spacing:.3px; color:rgba(var(--ink-rgb),0.45);">' + reassure + '</span>' +
            '</div>' +
          '</div>' +
        '</div>' +
        ...
        '<div style="position:relative; z-index:6; flex:none; padding:8px 26px calc(20px + env(safe-area-inset-bottom));">' +
          '<button id="wStart" style="appearance:none; border:none; cursor:pointer; width:100%; min-height:58px; border-radius:999px; background:#2775CA; border:2px solid var(--border-ink); display:flex; align-items:center; justify-content:center; gap:9px; box-shadow:3px 3px 0 rgba(var(--shadow-rgb),0.9);">' +
          ...
          '</button>' +
          '<div style="text-align:center; margin-top:14px;"><span id="wView" style="font-family:\'General Sans\',sans-serif; font-weight:500; font-size:14px; color:rgba(var(--ink-rgb),0.5); cursor:pointer;">view my account</span></div>' +
        '</div>' +
      '</div>';
